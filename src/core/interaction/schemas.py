...
class SetGoalPayload(BaseModel):
    """设定目标。"""
    type: str = Field(..., description="weight_loss / muscle_gain / maintenance / race / ...")
    target: Optional[dict[str, Any]] = None  # 按 type 不同，如 race: race_type, race_date, target_time, ...
...
class RecordStatusPayload(BaseModel):
    ...
    stress_level: Optional[int] = Field(None, ge=1, le=10)
    note: Optional[str] = Field(None, description="自由描述，如「今天很累」「心情一般」")
...
class ParsedRecord(BaseModel):
    """单条用户输入解析结果。"""
    intent: IntentType
    date: Optional[datetime.date] = Field(default=None, description="若用户未说日期则用当天")
    payload: Optional[dict[str, Any]] = Field(default=None, description="与 intent 对应的结构化数据")
